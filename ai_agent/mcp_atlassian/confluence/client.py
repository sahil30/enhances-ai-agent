...
class ConfluenceClient:
    ...
    def __init__(self, config: ConfluenceConfig | None = None) -> None:
        ...
        if self.config.auth_type == "oauth":
            ...
            logger.debug(
                f"Initializing Confluence client with Custom Token auth. "
                f"URL: {self.config.url}, "
                f"Token (masked): {mask_sensitive(str(self.config.custom_token))}"
            )
            # For custom endpoints, use token authentication with Bearer header
            session = Session()
            session.headers.update({
                "Authorization": f"Bearer {self.config.custom_token}",
            ...
            self.confluence = Confluence(
                url=self.config.url,
                session=session,
                cloud=False,  # Custom endpoints are typically not Atlassian Cloud
                verify_ssl=self.config.ssl_verify,
            )
        else:  # basic auth
            logger.debug(
